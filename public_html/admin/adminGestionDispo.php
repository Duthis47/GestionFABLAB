...
<body>
    ...
    <div class="container mb-5 mt-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2><?= $titreF ?></h2>
            <div>
                <a href="adminFonctBloque.php" class="btn <?= $isSalleMode ? 'btn-fablab-blue ' : 'btn-outline-fablab-blue' ?>">Salles</a>
                <a href="adminFonctBloque.php?estMateriel=true" class="btn <?= !$isSalleMode ? 'btn-fablab-blue' : 'btn-outline-fablab-blue' ?>">Matériels</a>
            </div>
        </div>
                ...
                <form method="POST" action="./../scriptAdmin/scriptBlocDeblocGroupe.php">
                    ...
                    <div class="mb-3">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <button type="button" class="btn btn-sm btn-outline-secondary" onclick="toggleAll('add')">Tout cocher / décocher</button>
                        </div>

                        <div class="border rounded p-2" style="max-height: 200px; overflow-y: auto;">
                            <?php foreach ($tableauElement as $element) : ?>
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="ids[]" value="<?= $element['idR'] ?>" id="add_<?= $element['idR'] ?>">
                    ...
                    <div class="table-responsive border rounded mb-3" style="max-height: 400px; overflow-y: auto;">
                        <table class="table table-hover align-middle mb-0">
                            <thead class="table-light sticky-top">
                                <tr>
                                    <th style="width: 50px;"><input type="checkbox" class="form-check-input" onclick="toggleAll('del')"></th>
                                    <th>Élément</th>
                                    <th>Début</th>
                                    <th>Fin</th>
                                </tr>
                            ...
                            <tbody>
                                <?php foreach ($blocagesActuels as $bloc) : ?>
                                    <tr>
                                        <td>
                                            <input type="checkbox" class="form-check-input check-del" name="toDelete[]" value="<?= $bloc['idR'] ?>|<?= $bloc['dateDebut'] ?>">
                                        </td>
        ...
        <div class="mt-3 text-center">
             <a href="afficherCalendrierAdmin.php<?= !$isSalleMode ? '?estMateriel=true' : '' ?>" class="btn btn-secondary">Retour au calendrier global</a>
        </div>
    </div>
